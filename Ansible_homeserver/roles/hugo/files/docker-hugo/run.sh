...
#         $HUGO --source="/src" --destination="$HUGO_DESTINATION" --baseURL="$HUGO_BASEURL" "$@" || exit 1
#     fi
#
...
while [ true ]
do
$HUGO server --source="/src" --destination="$HUGO_DESTINATION" --baseURL="$HUGO_BASEURL" --bind="0.0.0.0" "$@" || exit 1
done
